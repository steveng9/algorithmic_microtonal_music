...
function audio_buffer = build_audio_buffer(notes, start_times, durations, sound_func)
    fs = 44100;
    end_times = (start_times * fs) + (durations * fs);
    audio_buffer = zeros(1, round(max(end_times))+1); % Initialize the audio buffer

    
    for i = 1:length(notes)
        bell_sound = sound_func(notes(i), fs, durations(i));


        % Add fade-out to prevent clipping at the end of each note
        fade_samples = min(round(0.02 * fs), length(bell_sound));  % 20ms fade
        fade_out = ones(size(bell_sound));
        fade_out(end-fade_samples+1:end) = linspace(1, 0, fade_samples);
        bell_sound = bell_sound .* fade_out;
